"use strict";
var uni_modules_zebraUniappSwiperMaster_shared_classesToSelector = require("../../shared/classes-to-selector.js");
function Pagination({
  swiper,
  extendParams,
  on,
  ...
  function update() {
    ...
    if (params.type === "bullets" && swiper.pagination.bullets && swiper.pagination.bullets.length > 0) {
      ...
      bullets.forEach((item) => {
        item.removePaginationItemClass(["", "-next", "-next-next", "-prev", "-prev-prev", "-main"].map((suffix) => `${params.bulletActiveClass}${suffix}`).join(" "));
      });
      if ($el.length > 1) {
        bullets.each((bullet) => {
          const $bullet = $(bullet);
  ...
  function destroy() {
    ...
    swiper.pagination.$el;
    if (params.clickable) {
      swiper.off("paginationItemClick", uni_modules_zebraUniappSwiperMaster_shared_classesToSelector.classesToSelector(params.bulletClass));
    }
  }
  on("init update", () => {
